  ...
  server: {
    handlers: {
      GET: async () => {
        ...
        // 1. Fetch active tours from PostgreSQL (minimal payload)
        ...
        try {
          ...
          console.warn("[sitemap.xml] Error querying active tours:", err);
        }

        const today = new Date().toISOString().split("T")[0];

        // 2. Core static marketplace routes
        const staticRoutes = [
        ...
        // 4. Append dynamic tour catalog routes with DB lastmod
        if (tours.length > 0) {
          tours.forEach((tour) => {

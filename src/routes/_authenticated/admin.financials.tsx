...
export function AdminFinancials() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // 1. Fetch vendor profiles for subscription recurring revenues
      const { data: profiles } = await supabase
        .from("profiles")
        .select("id, full_name, company_name, email, subscription_tier, updated_at, created_at");

      // 2. Fetch custom lead unlocks / purchases
      ...
      const daysCount = period === "30d" ? 30 : period === "90d" ? 90 : 180;
      ...
      if (breakdownPieData.length === 0) {
        breakdownPieData.push({ name: "Demo Subscriptions", value: 10000, color: "#10b981" });
      }

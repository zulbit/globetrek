...
export const createLeadUnlockCheckout = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (pendingRows?.[0]?.payment_intent_id) {
      ...
      const existingSafepayUrl = `${baseUrl}/io/quick-link?ql=${existingTracker}`;
      const existingUrl = `/vendor/checkout?safepayUrl=${encodeURIComponent(existingSafepayUrl)}&leadId=${encodeURIComponent(data.leadId)}&type=tour&amount=5000&note=${encodeURIComponent("Unlock Custom Tour Lead")}&trackerId=${encodeURIComponent(existingTracker)}`;
    ...
    // Create SafePay QuickLink v2 (generates a real checkout page)
    ...
    try {
      ...
      if (qlRes.ok) {
        ...
        console.warn("[createLeadUnlockCheckout] SafePay QuickLink API failed:", await qlRes.text());
      }
    } catch (apiErr) {
      console.warn("[createLeadUnlockCheckout] SafePay QuickLink API call failed:", apiErr);
    }

    // Build internal checkout URL — our in-app page shows pre-filled vendor details + SafePay pay button
    const checkoutUrl = `/vendor/checkout?safepayUrl=${encodeURIComponent(safepayUrl)}&leadId=${encodeURIComponent(data.leadId)}&type=tour&amount=5000&note=${encodeURIComponent(noteText)}&trackerId=${encodeURIComponent(trackerId)}`;
